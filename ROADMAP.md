...
- [x] **[HIGH] `ProcessedStripeEvent` table cleanup cron** — done: `pruneProcessedStripeEvents()` runs nightly (`@Cron(CronExpression.EVERY_DAY_AT_MIDNIGHT, { timeZone: 'Europe/Warsaw' })`, Redis-locked) and deletes rows older than 7 days (`backend/src/modules/payments/payments.service.ts:1138-1149`). Checklist tag was stale (verified 2026-06-24).
- [x] **[HIGH] `processedStripeEvent` insert must be atomic with payment handler** — done: every handler (`markSessionPaid`, `markSessionFailed`, dispute/payout handlers) inserts its `processedStripeEvent` row inside the same `$transaction` as the state change it guards (`backend/src/modules/payments/payments.service.ts:357-363` and siblings). Checklist tag was stale (verified 2026-06-24).
...
- [ ] **[HARD GATE — bot protection]** Register a Cloudflare Turnstile site at [dash.cloudflare.com](https://dash.cloudflare.com) → Turnstile → Add site → select your production domain. Then set two env vars: `CLOUDFLARE_TURNSTILE_SECRET_KEY` (secret key) in Railway and `TURNSTILE_SITE_KEY` (site key) in Vercel → both shown on the Turnstile site detail page after creation. `frontend/scripts/inject-turnstile.mjs` runs during `prebuild` and patches `frontend/src/environments/environment.prod.ts:5` (`turnstileSiteKey`) with the Vercel env var at build time — trigger a Vercel redeploy after setting it. **Code-level safeguards now in place:** the backend guard (`backend/src/common/guards/turnstile.guard.ts`) throws `403` fail-closed in production when the secret key is absent (rather than passing all requests through); `TurnstileService.getToken()` (`frontend/src/app/core/services/turnstile.service.ts`) throws an error in production when the site key is blank instead of silently resolving to `''`. Without both keys registered and deployed, `POST /cart/items` and `POST /orders` are unprotected — scalpers can drain stock in seconds.
