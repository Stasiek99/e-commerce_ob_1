...
- [ ] **[HARD GATE — bot protection]** Register a Cloudflare Turnstile site at [dash.cloudflare.com](https://dash.cloudflare.com) → Turnstile → Add site → select your production domain. Then set two env vars: `CLOUDFLARE_TURNSTILE_SECRET_KEY` (secret key) in Railway and `TURNSTILE_SITE_KEY` (site key) in Vercel → both are shown on the Turnstile site detail page after creation. Without `CLOUDFLARE_TURNSTILE_SECRET_KEY`, `config.validation.ts` will boot-reject the backend in production; without `TURNSTILE_SITE_KEY`, the frontend sends no challenge token and bots can hammer `POST /cart/items` and `POST /orders` unchallenged — scalpers can drain stock in seconds.
