...
- `sweepOrphanedPendingOrders` (run at the end of every `reconcilePendingPayments` tick) read-then-restored stock/coupon usage with no claim step, unlike every sibling cancellation path — a second tick after the 540s cron lock TTL expired mid-run on a large backlog could double-restore stock and double-decrement `coupons.current_uses` — fixed by wrapping each order in the existing `refund-lock:<id>` and claiming it via a conditional `updateMany` keyed on `PENDING_PAYMENT` before touching stock/coupons, mirroring `OrdersService.updateStatus`/`OutboxProcessorService.recoverPendingMessages` (round 20)
...
- `GET /orders/:id/events` (order status-history timeline) has a complete backend implementation and zero frontend consumer — no security/data-integrity impact, low priority, not worth a dedicated fix on its own (round 20, open)
...
- `ReturnsService`/`ProductsService` bypass BullMQ retry queue (direct `EmailService` injection) — **resolved**: both now inject `EmailQueueService`; the raw `EmailService` is imported only within the `email` module itself, so every dispatch path gets retry/backoff, a deterministic `jobId`, and bounce-suppression checking (verified round 20)
...
- Corrective invoices (`faktura korygująca`) were fully generated and stored after a partial cancellation via a working `GET /orders/:id/corrective-invoice` endpoint, but nothing in the product (UI or email) ever told the customer one existed — a real document-delivery compliance gap, not just a missing convenience feature — fixed by adding a "Pobierz korektę" button to `order-detail.component.ts`, shown whenever any order item has `cancelledQuantity > 0` (round 20)
...
- `new-order-notification`/`low-stock-alert` templates rendered `customerEmail`/item names/SKUs straight into admin-bound HTML with no `escapeHtml` import — the same sibling-miss gap round 17's sweep was supposed to prevent via `xss-escaping.spec.ts`, which never grew to cover either file; `order-confirmation` turned out to have an identical miss on item names, found in the same pass — fixed for all three, and the spec now discovers every `*.template.ts` file via `fs.readdirSync` and fails if any lacks a coverage entry, instead of relying on a hand-maintained list (round 20)
- Resend bounce/complaint webhook's `createHash('sha256').update(to)` threw `TypeError` whenever `data.to` was empty/absent — the crash landed after the `EmailLog` row was written but before the suppression flag, so Resend's retries 500'd on the same event forever and the address was never suppressed — fixed with a null guard (`to ? createHash(...).digest('hex').slice(0,12) : 'unknown'`), matching the guards already present on the two DB writes immediately below each hash call (round 20)
- The GDPR retention cron only ever matched `outboxMessage` rows with `status: 'PROCESSED'`, so a row that exhausted all 3 retries and landed in `FAILED` was retained forever; `email-dlq` BullMQ jobs (kept indefinitely by design for human inspection) had no retention bound at all — fixed by setting `processedAt` on `FAILED` rows so they age out under the same cutoff, broadening the cron's `where` clause to `{ in: ['PROCESSED', 'FAILED'] }`, and adding a weekly `queue.clean()` sweep for the DLQ (round 20)
- `orders.service.ts`'s ten fire-and-forget notification dispatch sites only logged via `logger.warn`, unlike the structurally identical pattern in `payments.service.ts` which also calls `Sentry.captureException` — the gap was narrow (BullMQ-attempt failures after `queue.add()` succeeds are already monitored by the worker's own `failed` handler; only `EmailQueueService.enqueue()` throwing *before* `queue.add()` was invisible) but fixed by adding the same `Sentry.captureException` alongside each `logger.warn` (round 20)
- `outbox-recovery`'s lock TTL (25s, tied to the 30s `@Interval` period) only refreshed *after* each message completed, leaving the first message in a batch unprotected if it alone (e.g. an invoice PDF render + Supabase upload after a cold start) outlived the lock before the loop ever reached a refresh — fixed by refreshing immediately before each message's work and raising the TTL to 60s, sized to a single message's realistic worst case rather than the batch interval (round 20)
...
- `CartService` never loaded cart data during SSR (cart fetch unconditionally skipped server-side), so `checkoutGuard`'s empty-cart redirect to `/cart` had nothing bridging the guard's `UrlTree` to a real HTTP response — Angular silently rendered `/cart`'s empty-state markup under the `/checkout` URL at status 200. Full SSR cart hydration wasn't viable (guest-cart `sessionId` lives only in browser `localStorage`, and `auth.refresh()` is intentionally skipped during SSR) — fixed instead with `bridgeGuardRedirectsToHttp()` in `app.config.ts`, which turns any guard-driven `UrlTree` redirect into a real 302 via the `RESPONSE` token already used for product-detail's 404 case (round 20)
...
- Product detail 404 not handled — SSR returns blank 200 (soft-404) — later: the wildcard `**` route's `NotFoundComponent` had the identical gap (no `RESPONSE`/`.status(404)` call), so every other invalid URL on the site — typos, dead links, anything not specifically the product-detail lookup-fails path — also soft-404'd at HTTP 200, indexable by search engines and indistinguishable from a real page to status-checking tools — fixed by injecting `RESPONSE` and calling `.status(404)` in `ngOnInit` when `isPlatformServer`, mirroring the product-detail pattern (round 20)
...
- `build-and-test`, the CI job gating every merge, had no `timeout-minutes` at any level — unlike its sibling `e2e` job, which round 17 already hardened — a hang anywhere in its 9 steps (a frozen `pnpm audit` network call, a wedged Jest/fake-async test) fell through to GitHub Actions' 360-minute default job cap instead of failing fast, blocking `e2e` (which `needs: build-and-test`) from ever starting for up to 6 hours — fixed with `timeout-minutes: 20` (round 20)
...
This list spans ~408 distinct findings (20 folded in from round 16 — the 5 round-16 LOW-severity items were fixed directly on that round's branch; the other 15 CRITICAL/HIGH/MEDIUM findings remain open — plus 16 folded in from round 17, all fixed directly on that round's branch — plus 13 folded in from round 18, all fixed directly on that round's branch — plus 8 folded in from round 19, all fixed directly on that round's branch — plus 11 folded in from round 20, 10 fixed directly on that round's branch and 1 (the dead `GET /orders/:id/events` contract) left open as low-priority). Each new audit round should avoid restating any of the above and focus on genuinely new angles.
