...
🟡 MEDIUM — scrollPositionRestoration requires hybrid configuration (1/5 agents)
File: frontend/src/app/app.config.ts
Problem: The current configuration withInMemoryScrolling({ scrollPositionRestoration: 'top' }) forces a scroll to the top on every single navigation. In the product catalog, when a user clicks the "Back" button from a product detail page, they lose their previous scroll position on the list. This is a major UX issue often leading to "back-button abandonment," especially on mobile devices.
Requirements:
Product Catalog: When navigating back to the list, the scroll position must be restored (UX standard).
Homepage (/): For business reasons, we require that every time a user navigates to the homepage (whether via the menu or by pressing back), the page must automatically scroll to the top.
Fix: Change the main configuration to 'enabled' to fix the behavior in the catalog:
