...
| 24 | "Concurrent 401 refresh races across tabs can orphan a token" | **Fixed — regression test added.** See §5 above — the existing 30-second grace window plus per-tab `shareReplay` dedupe handles the realistic race. `auth.service.spec.ts`'s "keeps both tabs authenticated when TabB redeems the same stale cookie inside the grace window" test now drives two real sequential `refresh()` calls from one starting token and asserts both tabs end up authenticated with no family-wide theft revocation. |
...
findings (A1 fixed, A2 left as a tracked non-bug), 1 "largely resolved," now closed
with a regression test (#24).** This mirrors the exact staleness pattern `audit-exclusion-list.md`'s
