...
describe("AnalyticsService", () => {
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        { provide: PLATFORM_ID, useValue: platformId },
        {
          ...
          useValue: makeConsentMock(analyticsConsented),
        },
      ],
    });
  ...
  afterEach(() => {
    document.querySelectorAll("script").forEach((el) => {
      if ((el as HTMLScriptElement).src?.includes("googletagmanager.com"))
        el.remove();
    });
  ...
  describe("browser", () => {
    ...
    describe("init()", () => {
      it("is a no-op when gtmId is empty string — no GTM script injected", () => {
        ...
        svc.init("");
        const gtmScript = Array.from(
          document.head.querySelectorAll("script"),
        ).find((s) =>
          (s as HTMLScriptElement).src?.includes("googletagmanager.com"),
        );
      ...
      it("injects an external <script src> into <head> pointing at the GTM container ID", () => {
        ...
        const gtmScript = Array.from(
          document.head.querySelectorAll("script"),
        ).find((s) => (s as HTMLScriptElement).src?.includes("GTM-TEST123")) as
      ...
      it("does not inject GTM script when analytics consent is not granted", () => {
        ...
        const gtmScript = Array.from(
          document.head.querySelectorAll("script"),
        ).find((s) => (s as HTMLScriptElement).src?.includes("GTM-BLOCKED"));
        expect(gtmScript).toBeUndefined();
      });
      ...
      it("proceeds normally for a real container ID that starts with GTM- but not GTM-XXX", () => {
        ...
        const gtmScript = Array.from(
          document.head.querySelectorAll("script"),
        ).find((s) => (s as HTMLScriptElement).src?.includes("GTM-ABC1234"));
    ...
    describe("trackViewItem()", () => {
      ...
      it("always sends quantity: 1 regardless of params", () => {
        ...
        const items = (
          (window.dataLayer[1] as Record<string, unknown>)[
            "ecommerce"
