...
import { TuiButton, TuiLabel, TuiIcon, TuiInput } from "@taiga-ui/core";
      ...
      @for (addr of addressList; track addr.id) {
        @if (editingId() === addr.id) {
          ...
          >
            ...
            @if (addr.company) {
              <p class="addr-detail">{{ addr.company }}</p>
            }
            ...
            @if (confirmingDeleteId() === addr.id) {
              ...
              <div class="addr-actions">
                ...
                <button
                  ...
                  class="btn-delete"
                  [disabled]="!!working()"
                  (click)="startDeleteConfirm(addr.id)"
                >
                  Usuń
                </button>
              </div>
...
export class AddressesComponent implements OnInit {
  ...
  get addCities(): string[] {
    return this.addCitySuggestions();
  }
  get editCities(): string[] {
    return this.editCitySuggestions();
  }
  ...
  readonly addStreetStatus = signal<
    "idle" | "checking" | "found" | "not-found"
  >("idle");
  readonly editStreetStatus = signal<
    "idle" | "checking" | "found" | "not-found"
  >("idle");
  ...
  private buildForm() {
    return this.fb.group({
      ...
      street: [
        "",
        [Validators.required, Validators.maxLength(200), streetValidator],
      ],
      postalCode: [
        "",
  ...
  ngOnInit(): void {
    ...
    this.initPostalLookup(
      this.addForm,
      this.addCitySuggestions,
      this.addCityLoading,
    );
    this.initPostalLookup(
      this.editForm,
      this.editCitySuggestions,
      this.editCityLoading,
    );
  ...
  private initPostalLookup(
    ...
    form
      ...
      .valueChanges.pipe(
        tap((val) => {
          if (!/^\d{2}-\d{3}$/.test(val ?? "")) suggestions.set([]);
        }),
        debounceTime(500),
        distinctUntilChanged(),
  ...
  private initStreetCheck(
    ...
    merge(form.get("street")!.valueChanges, form.get("city")!.valueChanges)
      .pipe(
        tap(() => status.set("idle")),
        debounceTime(1200),
        map(() => ({
        ...
        filter(
          ({ street, city }) => !!street && !!city && form.get("street")!.valid,
        ),
        distinctUntilChanged(
  ...
  startEdit(addr: Address): void {
    ...
    if (addr.phone) {
      try {
        ...
        if (parsed?.country)
          this.editIsoCode = parsed.country as TuiCountryIsoCode;
      } catch {
        /* ignore */
      }
    }
  ...
  submitEdit(id: string): void {
    ...
    this.http
      .patch<Address>(
        `${environment.apiUrl}/users/me/addresses/${id}`,
        this.editForm.getRawValue(),
      )
      .subscribe({
        next: (updated) => {
          this.addresses.update((l) =>
            l.map((a) => (a.id === id ? updated : a)),
          );
          this.editingId.set(null);
  ...
  setDefault(id: string): void {
    this.working.set(id);
    this.http
      .patch<Address>(`${environment.apiUrl}/users/me/addresses/${id}`, {
        isDefault: true,
      })
      .subscribe({
        next: () => {
