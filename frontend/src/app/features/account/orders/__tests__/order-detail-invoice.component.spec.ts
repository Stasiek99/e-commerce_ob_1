...
describe('OrderDetailComponent — invoice download', () => {
  ...
  describe('canDownloadInvoice', () => {
    ...
    it.each(['PAID', 'PROCESSING', 'SHIPPED', 'DELIVERED', 'REFUNDED', 'PARTIALLY_REFUNDED'])(
      'returns true for %s',
      (status) => {
        const { component } = setup();
  ...
  describe('template — invoice button visibility', () => {
    ...
    it('does not render the invoice button for PENDING_PAYMENT', () => {
      ...
      fixture.detectChanges();
      httpMock.expectOne(`${API}/orders/${ORDER_ID}`).flush(makeOrder('PENDING_PAYMENT'));
      fixture.detectChanges();
      httpMock.verify();
    ...
    it('does not render the invoice button for CANCELLED', () => {
      ...
      fixture.detectChanges();
      httpMock.expectOne(`${API}/orders/${ORDER_ID}`).flush(makeOrder('CANCELLED'));
      fixture.detectChanges();
      httpMock.verify();
