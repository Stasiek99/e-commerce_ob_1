...
describe('OrderDetailComponent — trackingUrl()', () => {
  ...
  it('returns null for an unknown carrierCode', () => {
    const { component } = setup();
    expect(component.trackingUrl({ trackingNumber: '123', carrierCode: 'FEDEX' })).toBeNull();
  });

  it('builds the InPost tracking URL', () => {
    const { component } = setup();
    const url = component.trackingUrl({ trackingNumber: '123456789012', carrierCode: 'INPOST' });
    expect(url).toBe('https://inpost.pl/sledzenie-przesylek?number=123456789012');
  });

  it('builds the DHL tracking URL', () => {
    const { component } = setup();
    const url = component.trackingUrl({ trackingNumber: '1234567890', carrierCode: 'DHL' });
    expect(url).toBe('https://www.dhl.com/pl-pl/home/tracking.html?tracking-id=1234567890');
  });

  it('builds the GLS tracking URL', () => {
    const { component } = setup();
    const url = component.trackingUrl({ trackingNumber: '987654321', carrierCode: 'GLS' });
