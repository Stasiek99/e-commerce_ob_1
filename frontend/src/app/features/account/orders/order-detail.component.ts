  ...
  template: `
    @if (order()) {
      <div class="page">
        ...
        <!-- ── Invoice download ───────────────────────────── -->
        @if (
          canDownloadInvoice(order()!.status) || hasCorrectiveInvoice(order()!)
        ) {
        ...
        <div class="items-card">
          @for (item of order()!.items; track item.id) {
            <div
              class="item"
              [class.item--cancelled]="item.cancelledQuantity >= item.quantity"
            >
                  ...
                  <span class="item-cancelled-badge">
                    {{
                      item.cancelledQuantity === item.quantity
                        ? "anulowano"
                        : "anulowano " + item.cancelledQuantity
                    }}
                  </span>
                }
              </span>
              <span class="item-price">{{
                item.snapshotPrice * item.quantity | price
              }}</span>
            </div>
          }
          <div class="totals">
            ...
            @if (order()!.refundedAmountInCents > 0) {
              <div class="totals-row totals-refund">
                Zwrócono:
                <span>−{{ order()!.refundedAmountInCents | price }}</span>
              </div>
            }
        ...
        @if (canCancel(order()!.status)) {
          <div class="cancel-zone">
            @if (!confirming()) {
              ...
              <div class="confirm-box">
                @if (order()!.status === "PENDING_PAYMENT") {
                  <p>
                    Czy na pewno chcesz anulować to zamówienie? Żadna płatność
                    nie zostanie pobrana.
                  </p>
                } @else {
                  <p>
                    Czy na pewno chcesz zrezygnować z zamówienia? Pełna kwota
                    <strong>{{ order()!.totalInCents | price }}</strong>
                    zostanie zwrócona w ciągu 5–10 dni roboczych zgodnie z
            ...
            @if (
              ...
              <p class="legal-note">
                Prawo odstąpienia od umowy (ustawa z dnia 30 maja 2014 r. o
                prawach konsumenta)
              </p>
            }
          </div>
        ...
        @if (partialCancelling()) {
          <div tuiCardLarge class="partial-cancel-card">
              ...
              @for (line of partialLines; track line.orderItemId) {
                <div class="partial-item">
                  ...
                  @if (line.selected) {
                    ...
                    <span class="partial-item-price">{{
                      lineTotal(line) | price
                    }}</span>
                  } @else {
                    <span class="partial-item-max"
                      >maks. {{ line.maxQuantity }} szt.</span
                    >
                  }
                </div>
              }
            ...
            <div class="partial-summary">
              ...
              <strong class="partial-summary-amount">{{
                refundPreview() | price
              }}</strong>
            </div>
        ...
        @if (order()!.status === "SHIPPED" || order()!.status === "DELIVERED") {
          <p class="shipped-note">
            Zamówienie zostało już wysłane. Aby zgłosić zwrot, skontaktuj się z
            nami.
          </p>
        }
      </div>
...
export class OrderDetailComponent implements OnInit {
  ...
  trackingUrl(
    shipment:
      { trackingNumber?: string; carrierCode?: string } | null | undefined,
  ): string | null {
  ...
  hasCorrectiveInvoice(order: OrderDetail): boolean {
    return (
      !!order.invoiceUrl && order.items.some((i) => i.cancelledQuantity > 0)
    );
  }
  ...
  downloadInvoice(): void {
    ...
    this.http
      .get<{ invoiceUrl: string }>(`${environment.apiUrl}/orders/${id}/invoice`)
      .subscribe({
        next: ({ invoiceUrl }) => {
        ...
        error: (err) => {
          this.toast.error(
            err.error?.message ?? "Nie udało się wygenerować faktury.",
          );
          this.downloadingInvoice.set(false);
        },
      });
  ...
  downloadCorrectiveInvoice(): void {
    ...
    this.http
      .get<CorrectiveInvoiceResponse>(
        `${environment.apiUrl}/orders/${id}/corrective-invoice`,
      )
      .subscribe({
        ...
        error: (err) => {
          this.toast.error(
            err.error?.message ?? "Nie udało się pobrać faktury korygującej.",
          );
          this.downloadingCorrectiveInvoice.set(false);
        },
      });
  ...
  lineTotal(line: PartialCancelLine): number {
    ...
    const maxItemDiscount = Math.round(
      orderItem.snapshotPrice * fraction * orderItem.quantity,
    );
    const alreadyAppliedDiscount = orderItem.cancelledDiscountInCents ?? 0;
    const remainingItemDiscount = Math.max(
      0,
      maxItemDiscount - alreadyAppliedDiscount,
    );
    const wantedDiscount = Math.round(
      orderItem.snapshotPrice * fraction * line.quantity,
    );
  ...
  doPartialCancel(): void {
    ...
    this.http
      .post(`${environment.apiUrl}/orders/${id}/cancel-items`, { items })
      .subscribe({
        next: () => {
          ...
          this.toast.success(
            "Wybrane produkty zostały anulowane. Zwrot pojawi się w ciągu 5–10 dni roboczych.",
          );
          this.load();
        },
        error: (err) => {
          this.toast.error(
            err.error?.message ?? "Nie udało się anulować wybranych produktów.",
          );
          this.submittingPartial.set(false);
          this.actionInFlight.set(false);
        },
  ...
  doCancel(): void {
    ...
    this.http
      .post(`${environment.apiUrl}/orders/${id}/cancel`, body)
      .subscribe({
        next: () => {
        ...
        error: (err) => {
          this.toast.error(
            err.error?.message ?? "Nie udało się anulować zamówienia.",
          );
          this.cancelling.set(false);
          this.actionInFlight.set(false);
        },
