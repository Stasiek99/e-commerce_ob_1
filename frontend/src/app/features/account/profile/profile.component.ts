    ...
    <div class="page">
      ...
      @if (!editing()) {
        <div tuiCardLarge class="profile-card">
          ...
          <div class="info-row">
            ...
            <span class="info-value">{{
              formatPhone(auth.currentUser()?.phone ?? "")
            }}</span>
          </div>
          <div class="info-row">
      ...
      @if (editing()) {
        ...
        >
          ...
          @if (
            form.controls.nip.errors?.["pattern"] &&
            (form.controls.nip.dirty || form.controls.nip.touched)
          ) {
            <p class="field-error" role="alert">
      ...
      <div tuiCardLarge class="security-card">
        ...
        @if (!changingEmail()) {
          ...
          >
            ...
            <p class="info-text">
              Na nowy adres wyślemy link potwierdzający. Zmiana zacznie działać
              po jego kliknięciu.
            </p>
            <div class="form-actions">
              <button
      ...
      <div tuiCardLarge class="export-card">
        ...
        <div class="info-row">
          <span class="info-label"
            >Pobierz kopię swoich danych (RODO, art. 20)</span
          >
          <button
            tuiButton
            appearance="secondary"
        ...
        @if (!confirmingDelete()) {
          <div class="danger-card">
            <div class="danger-info">
              ...
              <p>
                Trwale usuwa Twoje konto, dane osobowe i historię zakupów. Tej
          ...
          <div class="danger-card danger-card--warning">
            <p class="danger-warning">
              ...
              Wszystkie Twoje dane zostaną trwale usunięte. Zamówienia zostaną
              zanonimizowane zgodnie z RODO.
            </p>
            @if (!isGoogleUser()) {
            ...
            <div class="danger-actions">
              ...
              <button
                ...
                class="btn-danger"
                [disabled]="
                  deleting() || (!isGoogleUser() && !deletePassword())
                "
                (click)="deleteAccount()"
              >
...
export class ProfileComponent {
  ...
  readonly form = this.fb.group({
    firstName: [
      ...
      [Validators.maxLength(50), nameValidator],
    ],
    lastName: [
  ...
  nameError(field: "firstName" | "lastName"): string | null {
    ...
    if (ctrl.errors?.["nameInvalid"])
      return "Tylko litery, myślniki i apostrofy";
  ...
  submitEmailChange(): void {
    ...
    this.http
      .patch(`${environment.apiUrl}/users/me/email`, { email, currentPassword })
      .subscribe({
        next: () => {
          this.toast.success(
        ...
        error: (err) => {
          this.toast.error(
            err.error?.message ?? "Nie udało się zmienić adresu e-mail.",
          );
          this.emailLoading.set(false);
        },
      });
  ...
  submitPasswordChange(): void {
    ...
    this.http
      .patch(`${environment.apiUrl}/users/me/password`, {
        currentPassword,
        newPassword,
      })
      .subscribe({
        ...
        error: (err) => {
          this.toast.error(
            err.error?.message ?? "Nie udało się zmienić hasła.",
          );
  ...
  exportData(): void {
    ...
    this.http
      .get(`${environment.apiUrl}/users/me/data-export`, {
        responseType: "blob",
      })
      .subscribe({
        next: (blob) => {
  ...
  deleteAccount(): void {
    ...
    this.http.delete(`${environment.apiUrl}/users/me`, { body }).subscribe({
      ...
      error: (err) => {
        this.toast.error(
          err.error?.message ?? "Błąd usuwania konta. Spróbuj ponownie.",
        );
