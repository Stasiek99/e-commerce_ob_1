...
@Component({
  ...
  template: `
    <div class="auth-page">
      ...
      >
        <header tuiHeader>
          <h1 tuiTitle>Resetuj hasło</h1>
        </header>

        @if (sent) {
          <p class="info-text">
            Jeśli ten adres email jest zarejestrowany w naszym sklepie, wyślemy na niego link do resetowania hasła. Sprawdź skrzynkę odbiorczą.
          </p>
          <a tuiButton appearance="secondary" [routerLink]="['/auth/login']" class="btn-full">
            Wróć do logowania
          </a>
        } @else {
          <p class="info-text">
            Podaj adres email powiązany z Twoim kontem, a wyślemy Ci link do ustawienia nowego hasła.
          </p>
