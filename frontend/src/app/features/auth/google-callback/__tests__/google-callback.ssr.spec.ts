...
function mockCart() {
  return {
    mergeWithServer: jest.fn().mockReturnValue(of({})),
    loadCart: jest.fn(),
  };
}
...
function createComponentWithReturnTo(
...
describe("GoogleCallbackComponent — SSR platform guard", () => {
  ...
  it("navigates to /auth/login when exchangeOAuthToken() errors in the browser", () => {
    const exchangeMock = jest
      .fn()
      .mockReturnValue(throwError(() => new Error("OAuth failed")));
    const router = { navigate: jest.fn(), navigateByUrl: jest.fn() };
    jest
      .spyOn(Storage.prototype, "getItem")
      .mockImplementation((key: string) => {
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        {
          provide: AuthService,
          useValue: { exchangeOAuthToken: exchangeMock },
        },
        { provide: CartService, useValue: mockCart() },
        { provide: Router, useValue: router },
      ],
