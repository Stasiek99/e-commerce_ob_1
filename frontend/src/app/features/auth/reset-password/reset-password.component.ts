      ...
      >
        ...
        @if (!token) {
          <p class="info-text error-text">
            Link jest nieprawidłowy lub wygasł. Poproś o nowy link do
            resetowania hasła.
          </p>
          <a
            tuiButton
          ...
          <tui-textfield>
            <label tuiLabel
              >Nowe hasło (min. 8 znaków, wielka i mała litera, cyfra)</label
            >
            <input
              tuiInput
              type="password"
  ...
  submit(): void {
    ...
    this.auth
      ...
      .subscribe({
        ...
        error: (err) => {
          this.toast.error(
            err.error?.message ?? "Link wygasł lub jest nieprawidłowy.",
          );
          this.loading = false;
