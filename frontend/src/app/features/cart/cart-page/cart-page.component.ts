...
import { TuiButton, TuiTitle, TuiCell } from "@taiga-ui/core";
            ...
            @for (item of cart.items(); track item.productVariantId) {
              <div tuiCell class="cart-cell">
                ...
                <div tuiTitle>
                  ...
                  <div tuiSubtitle>
                    {{ item.variantLabel }} ·
                    {{ item.priceInCents * item.quantity | price }}
                  </div>
                </div>
                <tui-counter
...
export class CartPageComponent {
  ...
  remove(variantId: string): void {
    const name = this.cart
      .items()
      .find((i) => i.productVariantId === variantId)?.productName;
    this.cart.removeItem(variantId).subscribe({
      next: () =>
        this.toast.info(
          name ? `Usunięto „${name}" z koszyka` : "Produkt usunięty z koszyka",
        ),
    });
  }
}
