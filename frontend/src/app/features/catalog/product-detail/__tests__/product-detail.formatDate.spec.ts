...
function setup() {
  ...
  TestBed.configureTestingModule({
    ...
    providers: [
      ...
      { provide: CartService, useValue: { addItem: jest.fn(), refreshFromServer: jest.fn() } },
      { provide: ToastService, useValue: { success: jest.fn(), error: jest.fn(), info: jest.fn() } },
      { provide: AnalyticsService, useValue: { trackAddToCart: jest.fn(), trackViewItem: jest.fn() } },
      { provide: SeoService, useValue: { updateProductMeta: jest.fn(), setProductJsonLd: jest.fn() } },
      { provide: WishlistService, useValue: { isInWishlist: jest.fn().mockReturnValue(false), toggle: jest.fn() } },
      { provide: StockStreamService, useValue: { connect: jest.fn().mockReturnValue(EMPTY) } },
      { provide: ReviewsService, useValue: { getByProduct: jest.fn().mockReturnValue(EMPTY), submit: jest.fn(), markHelpful: jest.fn() } },
    ],
  ...
  const component = TestBed.createComponent(ProductDetailComponent).componentInstance;
  return { component };
}

const fmt = (d: Date) =>
  new Intl.DateTimeFormat('pl-PL', { day: 'numeric', month: 'short', year: 'numeric' }).format(d);
...
describe('ProductDetailComponent.formatDate — UTC parsing invariant (Warsaw timezone)', () => {
  ...
  it('produces the same output for an ISO string and its equivalent Date object', () => {
    ...
    expect(component.formatDate(isoString)).toBe(component.formatDate(dateObject));
  });
