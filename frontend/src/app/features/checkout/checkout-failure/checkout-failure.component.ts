...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <p>
        Coś poszło nie tak. Spróbuj ponownie lub wybierz inną metodę płatności.
      </p>
...
export class CheckoutFailureComponent implements OnInit {
  ...
  readonly guestToken = signal<string | null>(
    this.route.snapshot.queryParamMap.get("guestToken"),
  );
  ...
  retryPayment(): void {
    ...
    this.http
      .post<{ paymentUrl: string }>(
        `${environment.apiUrl}/orders/${id}/retry-payment`,
        {},
        { params },
      )
      .subscribe({
        ...
        error: (err) => {
          ...
          this.retryError.set(
            err.error?.message ??
              "Nie udało się ponowić płatności. Spróbuj ponownie.",
          );
        },
      });
  }
  ...
  cancelOrder(): void {
    ...
    if (token) params = params.set("token", token);
    this.http
      .post(`${environment.apiUrl}/orders/${id}/cancel`, {}, { params })
      .subscribe({
        next: () => {
