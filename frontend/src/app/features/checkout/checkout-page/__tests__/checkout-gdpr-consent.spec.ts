  ...
  TestBed.configureTestingModule({
    ...
    providers: [
      ...
      },
      {
        ...
        useValue: { getToken: jest.fn().mockResolvedValue("") },
      },
    ],
  ...
  describe("Step 2 template — combined consent checkbox removed", () => {
    it("does not render the review-request consent checkbox in the summary step", () => {
      ...
      const labelTexts = labels.map(
        (l) => l.nativeElement.textContent as string,
      );
      const hasReviewLabel = labelTexts.some(
        (t) =>
          t.includes("ocenę") ||
          t.includes("marketingConsent") ||
          t.includes("wiadomości e-mail z prośbą"),
      );
