...
@Component({
          ...
          <div class="success">
            ...
            @if (submittedType() === "WITHDRAWAL") {
              <p class="success__legal">
                Niniejsze zgłoszenie stanowi jednoznaczne oświadczenie o
                odstąpieniu od umowy sprzedaży zgodnie z art.&nbsp;27 Ustawy z
                dnia 30 maja 2014&nbsp;r. o prawach konsumenta (Dz.U. 2014 poz.
                827). Oświadczenie zostało złożone w terminie 14 dni od odbioru
                towaru.
              </p>
            }
            ...
            @if (submittedType() === "WITHDRAWAL") {
              ...
              <p class="success__text">
                Odpowiemy na reklamację w ciągu <strong>14 dni</strong> od jej
                otrzymania. Brak odpowiedzi w tym terminie oznacza uznanie
                reklamacji (art.&nbsp;7a Ustawy o prawach konsumenta).
              </p>
            }
            <a
              routerLink="/"
          ...
          <form class="form" [formGroup]="form" (ngSubmit)="submit()">
              ...
              <div class="radio-group">
                ...
                >
                  ...
                  <span class="radio-option__desc">
                    Rezygnacja z zakupu w ciągu 14 dni od odbioru — bez podania
              ...
              <div class="alert alert--warning">
                ...
                <div>
                  <p class="alert__title">
                    Ważne — wyjątek dla produktów higienicznych (art.&nbsp;38
                    pkt&nbsp;5)
                  </p>
                  <p class="alert__body">
                    Prawo odstąpienia <strong>nie przysługuje</strong> dla
                    perfum, kosmetyków i produktów do pielęgnacji ciała
                    dostarczonych w zapieczętowanym opakowaniu,
                    <strong
                      >jeżeli opakowanie zostało otwarte po dostarczeniu</strong
                    >. Zwroty przyjmujemy wyłącznie w oryginalnym, nienaruszonym
                    opakowaniu z nienaruszoną folią/plombą.
                  </p>
                </div>
              </div>
            }
              ...
              <div>
                  ...
                  <label tuiLabel>
                    ...
                    @if (form.value.type === "WITHDRAWAL") {
                      <span class="label-hint">
                        — wymagana do weryfikacji terminu</span
                      >
                    }
                  </label>
                  <input
                ...
                @if (touched("deliveryDate")) {
                  <p class="field-error" role="alert">
                    Podaj datę odbioru towaru
                  </p>
                }

                <!-- 14-day deadline status -->
                @if (
                  form.value.type === "WITHDRAWAL" && form.value.deliveryDate
                ) {
                  @if (deadlineStatus() === "expired") {
                    <div class="deadline deadline--expired">
                      ...
                      Złożenie zgłoszenia po terminie nie jest skuteczne
                      prawnie.
                    </div>
              ...
              <div class="section">
                ...
                >
                  <input
                    type="checkbox"
                    tuiCheckbox
                    formControlName="sealIntact"
                  />
                  <span>
                    Potwierdzam, że zwracane produkty posiadają
                    <strong>nienaruszone, oryginalne opakowanie</strong> z
                    nietkniętą folią ochronną/plombą. Rozumiem, że zwrot
                    produktów z naruszonymi opakowaniami nie jest możliwy na
                    podstawie prawa odstąpienia od umowy.
                  </span>
                </label>
                @if (sealError()) {
                  <p class="field-error" role="alert">
                    Potwierdzenie stanu opakowania jest wymagane do złożenia
                    odstąpienia
                  </p>
                }
              </div>
              ...
              <div class="section">
                ...
                <div class="radio-group radio-group--compact">
                  @for (opt of resolutionOptions; track opt.value) {
                    <label
                      class="radio-option radio-option--sm"
                      [class.radio-option--active]="
                        form.value.requestedResolution === opt.value
                      "
                    >
                      <input
                        type="radio"
                        formControlName="requestedResolution"
                        [value]="opt.value"
                        class="sr-only"
                      />
                ...
                @if (
                  touched("requestedResolution") &&
                  !form.value.requestedResolution
                ) {
            ...
            @if (form.value.type === "WITHDRAWAL") {
              <div class="section">
                ...
                @if (
                  form.get("bankAccount")?.invalid &&
                  form.get("bankAccount")?.touched
                ) {
                  <p class="field-error" role="alert">
                    Nieprawidłowy format IBAN (np. PL61109010140000071219812874)
                  </p>
                }
                <p class="field-hint">
                  Jeśli płaciłeś/aś kartą lub BLIK, zwrot trafi automatycznie na
                  pierwotną metodę płatności — numer konta nie jest wtedy
                  potrzebny. Podaj go tylko, jeśli preferujesz przelew bankowy.
                </p>
              </div>
            }
            ...
            <div class="section">
              <label
                class="checkbox-row"
                [class.checkbox-row--error]="
                  touched('rodoConsent') && form.get('rodoConsent')?.invalid
                "
              >
                <input
                  type="checkbox"
                  tuiCheckbox
                  formControlName="rodoConsent"
                />
                <span>
                  Wyrażam zgodę na przetwarzanie moich danych osobowych przez
                  Aromaterie w celu rozpatrzenia zgłoszenia zwrotu/reklamacji,
                  zgodnie z
            ...
            <div class="form__footer">
              <p class="form__legal">
                ...
                <a routerLink="/legal/withdrawal" target="_blank"
                  >polityką zwrotów</a
                >
  ...
  styles: [
    `
      ...
      .field-error {
        font-size: 12px;
        color: var(--tui-status-negative, #d32f2f);
        margin: 4px 0 0;
      }
      .field-hint {
        font-size: 12px;
...
export class ReturnRequestComponent {
  ...
  readonly resolutionOptions = [
    {
      value: "REPAIR",
      label: "Naprawa produktu",
      desc: "Żądam usunięcia wady",
    },
    {
      value: "REPLACEMENT",
      label: "Wymiana na nowy",
      desc: "Żądam wymiany na produkt wolny od wad",
    },
    {
      value: "PRICE_REDUCTION",
      label: "Obniżenie ceny",
      desc: "Żądam stosownego obniżenia ceny",
    },
    {
      value: "REFUND",
      label: "Zwrot pieniędzy",
      desc: "Odstępuję od umowy i żądam zwrotu zapłaconej kwoty",
    },
  ];
  ...
  constructor() {
    ...
    this.form.get("type")!.valueChanges.subscribe((type) => {
      ...
      if (type === "COMPLAINT") {
        reasonCtrl.addValidators([
          Validators.required,
          Validators.minLength(10),
        ]);
      } else {
        reasonCtrl.removeValidators([
          Validators.required,
          Validators.minLength(10),
        ]);
      }
  ...
  submit(): void {
    ...
    const rawIban =
      val.bankAccount?.trim().replace(/\s+/g, "").toUpperCase() || undefined;
    const payload = {
      type: val.type,
      ...
      bankAccount: rawIban,
      ...(val.type === "WITHDRAWAL"
        ? { sealedOnReturn: val.sealIntact === true }
        : {}),
    };

    this.http
      .post<{ id: string }>(`${environment.apiUrl}/returns`, payload)
      .subscribe({
        next: (res) => {
