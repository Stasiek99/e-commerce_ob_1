...
describe('appConfig service-worker registration', () => {
  ...
  it('registers SW only when stable (not on first paint) with 30 s timeout', () => {
    expect(source).toContain("registrationStrategy: 'registerWhenStable:30000'");
  });
...
describe('push notifications (⚠️ not yet implemented)', () => {
  ...
  it.todo('requestSubscription() calls swPush.requestSubscription with VAPID key');
  it.todo('requestSubscription() POSTs subscription to /api/notifications/subscribe');
  it.todo('requestSubscription() handles NotAllowedError (user denied permission)');
  it.todo('requestSubscription() handles non-HTTPS environment gracefully');
  it.todo('unsubscribe() calls swPush.unsubscribe and DELETE /api/notifications/subscribe');
