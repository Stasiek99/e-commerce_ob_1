...
describe("FooterComponent", () => {
  ...
  describe("ODR platform link (EU Reg. 524/2013 Art. 14)", () => {
    function getOdrLink(fixture: ReturnType<typeof setup>["fixture"]) {
      return fixture.debugElement
        ...
        .find((el) =>
          (el.nativeElement as HTMLAnchorElement).href.includes(
  ...
  describe("UŚUDE Art. 5 — mandatory seller identity block", () => {
    ...
    it('seller block has aria-label "Dane sprzedawcy" for screen-reader accessibility', () => {
      ...
      expect(block.nativeElement.getAttribute("aria-label")).toBe(
        "Dane sprzedawcy",
      );
    });
    ...
    it("renders seller legal name from environment.seller", () => {
      ...
      const block = getSellerBlock(fixture);
      expect(block.nativeElement.textContent).toContain(
        environment.seller.legalName,
      );
    });
    ...
    it("renders contact email as a mailto: link", () => {
      ...
      const emailLink = fixture.debugElement.query(
        By.css("address.footer__seller a"),
      );
      expect(emailLink.nativeElement.getAttribute("href")).toBe(
        `mailto:${environment.seller.email}`,
      );
    });
    ...
    it("email link displays the email address as its visible text", () => {
      ...
      expect(emailLink.nativeElement.textContent.trim()).toBe(
        environment.seller.email,
      );
    });
