      ...
      <div class="footer__inner">
        ...
        <nav class="footer__legal" aria-label="Informacje prawne">
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            >Platforma ODR (rozwiązywanie sporów online)</a
          >
        </nav>
        <address class="footer__seller" aria-label="Dane sprzedawcy">
          ...
          <span
            >{{ seller.street }}, {{ seller.postalCode }}
            {{ seller.city }}</span
          >
          <span
            >NIP: {{ seller.nip }} &nbsp;·&nbsp; REGON: {{ seller.regon }}</span
          >
          <span>KRS/CEIDG: {{ seller.krs }}</span>
          <a [href]="'mailto:' + seller.email">{{ seller.email }}</a>
        </address>
        <p class="footer__copy">
          &copy; {{ year }} Aromaterie. Wszelkie prawa zastrzeżone.
        </p>
      </div>
    </footer>
  `,
  styles: [
    `
      ...
      .footer__seller {
        ...
        align-items: center;
        gap: 4px;
        /* This block carries the legally required seller details (company name,
         NIP, REGON, address). 11px was the smallest text on the site for the
         content that most needs to be readable. */
        font-size: 12px;
